...
export function MultiplayerSetup({ onBack, onJoinRoom }: Props) {
  ...
  const [pairsCount, setPairsCount] = useState<PairsCount>(8);
        ...
        {setupMode === 'HOST' && (
          <>
            <div className="game-panel">
              <h2 className="font-bold text-slate-800 mb-4">Board Size</h2>
              <div className="flex flex-col gap-2">
                {([6, 8, 12] as PairsCount[]).map(sz => (
                  <button 
                    key={sz}
                    onClick={() => setPairsCount(sz)} 
                    className={`w-full py-3 rounded-xl border-2 font-bold transition-all ${pairsCount === sz ? 'border-indigo-600 bg-indigo-50 text-indigo-700 scale-105' : 'border-slate-100 bg-white text-slate-600 hover:border-slate-200 hover:bg-slate-50'}`}
                  >
                    {sz} Pairs ({sz * 2} cards)
                  </button>
                ))}
