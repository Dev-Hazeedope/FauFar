...
import { subscribeToRoom, startGame, leaveRoom, makeMove, setGameCompleted, clientId, MPRoom } from './MultiplayerManager';
...
export function MultiplayerGameplay({ room: initialRoom, onLeave, onGameEnded }: Props) {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToRoom(room.id, (updatedRoom) => {
      ...
      // Play tap if opponent reveals a card
      if (prevRoom.revealedIds.length < updatedRoom.revealedIds.length && updatedRoom.turn !== myPlayer?.number) {
        audio.playTap();
      }
      ...
      // Play wrong if revealedIds went from 2 to 0 without matchedIds increasing
      if (prevRoom.revealedIds.length === 2 && updatedRoom.revealedIds.length === 0 && prevRoom.matchedIds.length === updatedRoom.matchedIds.length) {
        // Just play a small tap for turn transition if it wasn't a match
        // Or could play wrong, but we already play wrong on the client side when the mismatch happens.
      }
  ...
  const handleCardClick = (id: string) => {
    ...
    if (room.revealedIds.length === 2) return; // Wait for manual continue
    if (room.revealedIds.includes(id)) return;
    
    const newRevealed = [...room.revealedIds, id];
    
    if (newRevealed.length === 1) {
      // Just revealing the first card
      audio.playTap();
      makeMove(room.id, newRevealed, room.matchedIds, room.scores, room.turn, false);
      return;
    }
    ...
    if (c1 && c2 && c1.iconIdx === c2.iconIdx) {
      ...
      const newMatched = [...room.matchedIds, c1.id, c2.id];
      const newScores = { ...room.scores };
      newScores[myPlayer.number] += 1;
      
      const isCompleted = newMatched.length === room.cards.length;
      makeMove(room.id, [], newMatched, newScores, room.turn, isCompleted);
    } else {
      // No match
      audio.playWrong();
  ...
  const handleContinue = () => {
    if (room.state !== 'playing' || room.turn !== myPlayer?.number) return;
    if (room.revealedIds.length === 2) {
      const nextTurn = myPlayer.number === 1 ? 2 : 1;
      makeMove(room.id, [], room.matchedIds, room.scores, nextTurn, false);
    }
  };
      ...
      <main className="flex-1 overflow-y-auto p-4 flex flex-col items-center">
        <div className="w-full max-w-4xl flex-1 flex flex-wrap justify-center content-center gap-3 pb-8">
          {room.cards.map((card, i) => {
            const isRevealed = room.revealedIds.includes(card.id) || room.matchedIds.includes(card.id);
            const Icon = Array.from(SHARED_ICONS.values())[card.iconIdx];
            return (
              <button
                key={card.id}
                onClick={() => handleCardClick(card.id)}
                disabled={room.turn !== myPlayer?.number}
                className={`flex items-center justify-center w-16 h-20 sm:w-20 sm:h-24 rounded-xl transition-all shadow-sm border ${isRevealed ? 'bg-white border-slate-200 text-slate-800 scale-100' : 'bg-indigo-100 border-indigo-200 text-transparent scale-95 hover:scale-100 hover:bg-indigo-200 disabled:opacity-80 disabled:hover:scale-95'}`}
              >
        ...
        <div className="h-24 flex items-center justify-center shrink-0 w-full mb-8">
          {room.revealedIds.length === 2 && room.turn === myPlayer?.number && (
          ...
          {room.revealedIds.length === 2 && room.turn !== myPlayer?.number && (
            <div className="px-8 py-4 bg-slate-200 text-slate-500 font-bold rounded-full animate-in fade-in slide-in-from-bottom-2">
