            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200">
              ...
              <div className="flex gap-2">
                {[4, 6, 8].map(sz => (
                  <button 
                    key={sz}
                    onClick={() => setBoardSize(sz as BoardSize)} 
                    className={`flex-1 py-3 rounded-xl border-2 font-bold transition-all ${boardSize === sz ? 'border-indigo-600 bg-indigo-50 text-indigo-700 scale-105' : 'border-slate-100 bg-white text-slate-600 hover:border-slate-200 hover:bg-slate-50'}`}
                  >
                    {sz}x{sz}
                  </button>
                ))}
