...
import { subscribeToRoom, startGame, leaveRoom, clientId, CTCRoom } from './MultiplayerManager';

interface Props {
  room: CTCRoom;
  onLeave: () => void;
  onRoomUpdated: (room: CTCRoom) => void;
}
      ...
      <div className="flex-1 flex flex-col justify-center max-w-md mx-auto w-full gap-8">
        ...
        <div className="text-center flex flex-col items-center">
          ...
          {!room.isDraw && winnerPlayer && (
            <p className="text-indigo-600 font-bold mt-2 text-xl">took {room.guesses[winnerPlayer.number]} guesses</p>
          )}
        </div>
            ...
            {Object.values(room.players).sort((a: any, b: any) => (room.scores ? room.scores[b.number] - room.scores[a.number] : (room.guesses ? room.guesses[a.number] - room.guesses[b.number] : 0))).map((p: any) => (
              <div key={p.id} className="flex items-center justify-between p-4 rounded-xl bg-slate-50 border border-slate-100">
                ...
                <div className="flex items-center gap-4">
                  <span className="font-black text-slate-900">{room.guesses[p.number]} guesses</span>
                  <div className={`w-4 h-4 rounded-full ${p.number === 1 ? 'bg-indigo-600' : 'bg-rose-500'}`} />
                </div>
              </div>
            ))}
          </div>
          <div className="mt-6 text-center text-slate-500 font-bold">
            Secret Code was:<br/>
            <span className="text-slate-800 text-xl tracking-[0.5em] font-black">{room.secret.join('')}</span>
          </div>
        </div>
