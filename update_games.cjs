...
// 4. WhatsMissing
p('src/games/WhatsMissing/index.tsx', [
  { from: `import { Home, RefreshCw } from 'lucide-react';`, 
    to: `import { Home, RefreshCw } from 'lucide-react';\nimport { TimerSetup } from '../../components/TimerSetup';\nimport { TimerDisplay } from '../../components/TimerDisplay';` },
  { from: `const [phase, setPhase] = useState<Phase>('setup');`, 
    to: `const [phase, setPhase] = useState<Phase>('setup');\n  const [timedMode, setTimedMode] = useState(false);\n  const [timeLimit, setTimeLimit] = useState(60);\n  const [timeLeft, setTimeLeft] = useState(60);\n  const [isTimeUp, setIsTimeUp] = useState(false);` },
  { from: `  const startStudy = (total: number, missing: number) => {\n    setTotalItems(total);\n    setMissingCount(missing);`, 
    to: `  const startStudy = (total: number, missing: number) => {\n    setTimeLeft(timeLimit);\n    setIsTimeUp(false);\n    setTotalItems(total);\n    setMissingCount(missing);` },
  { from: `            <button onClick={() => startStudy(6, 1)} className="w-full py-4 bg-slate-100 hover:bg-indigo-50 text-slate-700 hover:text-indigo-700 font-bold rounded-xl transition-colors">6 Objects (1 Missing)</button>\n            <button onClick={() => startStudy(9, 1)} className="w-full py-4 bg-slate-100 hover:bg-indigo-50 text-slate-700 hover:text-indigo-700 font-bold rounded-xl transition-colors">9 Objects (1 Missing)</button>\n            <button onClick={() => startStudy(12, 2)} className="w-full py-4 bg-slate-100 hover:bg-indigo-50 text-slate-700 hover:text-indigo-700 font-bold rounded-xl transition-colors">12 Objects (2 Missing)</button>`, 
    to: `            <button onClick={() => startStudy(12, 2)} className="w-full py-4 bg-slate-100 hover:bg-indigo-50 text-slate-700 hover:text-indigo-700 font-bold rounded-xl transition-colors">12 Objects (2 Missing)</button>\n            <button onClick={() => startStudy(16, 3)} className="w-full py-4 bg-slate-100 hover:bg-indigo-50 text-slate-700 hover:text-indigo-700 font-bold rounded-xl transition-colors">16 Objects (3 Missing)</button>\n            <button onClick={() => startStudy(20, 4)} className="w-full py-4 bg-slate-100 hover:bg-indigo-50 text-slate-700 hover:text-indigo-700 font-bold rounded-xl transition-colors">20 Objects (4 Missing)</button>` },
  { from: `          <button onClick={onExit} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors">\n            <Home className="w-6 h-6" />\n          </button>\n          <h2 className="text-xl font-bold text-slate-800">What's Missing?</h2>\n        </div>`,
    to: `          <button onClick={onExit} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors">\n            <Home className="w-6 h-6" />\n          </button>\n          <h2 className="text-xl font-bold text-slate-800">What's Missing?</h2>\n        </div>\n        <TimerDisplay timedMode={timedMode} timeLeft={timeLeft} setTimeLeft={setTimeLeft} isActive={phase === 'study' || phase === 'guess'} onTimeUp={() => { setIsTimeUp(true); setPhase('completed'); }} />` },
  { from: `        <div className="flex-1 flex flex-col justify-center max-w-sm mx-auto w-full space-y-4">`,
